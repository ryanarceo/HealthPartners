...
spark = SparkSession.builder.appName("ClaimMedcToRaw").getOrCreate()

# Google Cloud Storage (GCS) Configuration
GCS_BUCKET = "db-health-partners"
CLAIM_TYPE = "claimmedc"
CONFIG_FILE_PATH = f"gs://{GCS_BUCKET}/configs/load_config.csv"


# BigQuery Configuration
BQ_PROJECT = "gcp-health-partners-473103"
...
# MySQL Configuration
MYSQL_CONFIG = {
    "url": "jdbc:mysql://34.121.181.77:3306/rawviewable_db?useSSL=false&allowPublicKeyRetrieval=true",
...
##------------------------------------------------------------------------------------------------------------------##
# Logging Mechanism Create place holder with no values. If there is a log generated in next step write an log to BQ
...
def move_files(table):
    blobs = list(storage_client.bucket(GCS_BUCKET).list_blobs(prefix=f"inputdate/{table}/"))
...
def get_latest_watermark(table_name):
    query = f"""
        ...
        FROM `{BQ_AUDIT_TABLE}`
        WHERE tablename = '{table_name}' and data_source = "claimmedc"
    """
...
def extract_and_save_to_raw(table, load_type, watermark_col):
    try:
        ...
        JSON_FILE_PATH = f"inputdata/CLAIM_TYPE/{table}/{table}_{today}.json"
        ...
        audit_df = spark.createDataFrame([
            ("claimmedc", table, load_type, df.count(), datetime.datetime.now(), "SUCCESS")], 
...
for row in config_df.collect():
    if row["is_active"] == '1': 
        ...
        move_files(table)
        extract_and_save_to_raw(table, load_type, watermark)
