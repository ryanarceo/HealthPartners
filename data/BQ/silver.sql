...
-- 1. Create table claims by Merge Data from Medical and RX  
CREATE TABLE IF NOT EXISTS `gcp-health-partners-473103.silver_dataset.claims` (
    Dept_Id STRING,
    SRC_Dept_Id STRING,
    Name STRING,
    datasource STRING,
...
-- 2. Truncate Silver Table Before Inserting Data cleanup
TRUNCATE TABLE `gcp-health-partners-473103.silver_dataset.departments`;

-- 3. full load by Inserting merged Data 
INSERT INTO `gcp-health-partners-473103.silver_dataset.departments`
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT *, 'medc' AS datasource FROM `gcp-health-partners-473103.bronze_dataset.departments_ha`
    UNION ALL
    SELECT DISTINCT *, 'rx' AS datasource FROM `gcp-health-partners-473103.bronze_dataset.departments_hb`
);
