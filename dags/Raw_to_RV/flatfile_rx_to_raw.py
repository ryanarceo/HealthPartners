...
spark = SparkSession.builder.appName("RXToRaw").getOrCreate()
...
def get_latest_watermark(table_name):
    query = f"""
        ...
        FROM `{BQ_AUDIT_TABLE}`
        WHERE tablename = '{table_name}' and data_source = "claimrx"
    """
...
def extract_and_save_to_raw(table, load_type, watermark_col):
    try:
        ...
        audit_df = spark.createDataFrame([
            ("claimrx", table, load_type, df.count(), datetime.datetime.now(), "SUCCESS")], 
...
for row in config_df.collect():
    if row["is_active"] == '1' and row["datasource"] == "claimrx": 
